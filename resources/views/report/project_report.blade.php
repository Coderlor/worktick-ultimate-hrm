...
<div class="breadcrumb">
    <h1> {{ __('translate.Projects') }}</h1>
    <ul>
        <li><a href="/report/project">{{ __('translate.Project_Report') }}</a></li>
        <li>{{ __('translate.Projects') }}</li>
    </ul>
</div>

<div class="separator-breadcrumb border-top"></div>

<div class="row" id="section_Project_report">
    <div class="col-md-12">
        <div class="card text-left">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="ul-contact-list" class="display table project_datatable">
                        <thead>
                            <tr>
        ...
        <div class="modal fade" id="Filter_Project_Modal" tabindex="-1" role="dialog"
            aria-labelledby="Filter_Project_Modal" aria-hidden="true">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form method="POST" id="Filter_project_report">
                            @csrf
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <label for="client_id" class="ul-form__label">{{ __('translate.Client') }} </label>
...
<script type="text/javascript">
    $(function () {
      "use strict";
        ...
        project_datatable();
        function project_datatable(title ='' , client_id ='' , company_id='' , priority='' , status=''){
            var dataTable = $('#ul-contact-list').DataTable({
                ...
                "order": [[ 0, "desc" ]],
                ajax: {
                    url: "{{ route('project_report_index') }}",
                    data: {
                ...
                columns: [
                    {data: 'title', name: 'Project'},
                    {data: 'client.username', name: 'Client'},
         ...
         $('#Clear_Form').on('click' , function (e) {
            ...
            project_datatable(title, client_id, company_id, priority, status);
        });

         // Show Modal Filter

        $('#Show_Modal_Filter').on('click' , function (e) {
            $('#Filter_Project_Modal').modal('show');
        });


         // Submit Filter

        $('#Filter_project_report').on('submit' , function (e) {
            e.preventDefault();
            var title = $('#title').val();
            ...
            $('#Filter_Project_Modal').modal('hide');
